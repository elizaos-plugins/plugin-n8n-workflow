...
import { listCredentialsAction } from "../../../src/actions/listCredentials";
...
describe("LIST_N8N_CREDENTIALS action", () => {
  test("lists credentials successfully", async () => {
    const mockService = createMockService();
    const runtime = createMockRuntime({
      services: { [N8N_WORKFLOW_SERVICE_TYPE]: mockService },
    });
    ...
    const result = await listCredentialsAction.handler(
      runtime,
      createMockMessage(),
      createMockState(),
      {},
  ...
  test("handles empty credential list", async () => {
    const mockService = createMockService({
      listCredentials: mock(() => Promise.resolve([])),
    });
    const runtime = createMockRuntime({
      services: { [N8N_WORKFLOW_SERVICE_TYPE]: mockService },
    });
    ...
    await listCredentialsAction.handler(
      runtime,
      createMockMessage(),
      createMockState(),
      {},
