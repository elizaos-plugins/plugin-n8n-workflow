...
} from "../../src/utils/credentialResolver";
...
function createMockOAuthService(
  overrides?: Partial<OAuthService>,
): OAuthService {
  return {
    ...
    getN8nCredId: mock(() => Promise.resolve(null)),
    setN8nCredId: mock(() => Promise.resolve()),
...
describe("resolveCredentials", () => {
  ...
  test("cloud mode: creates credentials via OAuth", () => {
    ...
    return resolveCredentials(
      ...
      expect(oauthService.setN8nCredId).toHaveBeenCalled();
    });
  });

  test("cloud mode: returns existing credential ID from cache", () => {
    const oauthService = createMockOAuthService({
      getN8nCredId: mock(() => Promise.resolve("cached-cred-id")),
    });
    const runtime = createMockRuntime({
      services: { oauth: oauthService },
    });
